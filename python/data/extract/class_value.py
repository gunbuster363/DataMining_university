#Program name : class_value.py
#Author       : LAM Wai Man
#Student ID   : 07010702
#Description  : To extract a list of reviews and their correspinding label for training of classifier.
#               The labels would be : good money value, bad money value
#Input        : name.txt
#Output       : value.dat dump in "../classification/value" location
#Usage        : $python class_value.py
...
while (neg < 990 or pos < 990 ):
  ...
  command = 'select review, value from ' + name + where + ';' 
  con.execute(command) 
  for i in con:
    str = i[0]
    ...
    if(i[1]>3 and pos<990):
     tuple = (tokens,"good money value")
     documents.append(tuple)
     pos+=1

    if(i[1]<3 and neg<990):
     tuple = (tokens,"bad money value")
     documents.append(tuple)
     neg+=1
...
for w in dict:
 print w, dict[w]
f1.close()
fout = open("../classification/value/value.dat", "wb")
