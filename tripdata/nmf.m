...
initgrad = sqrt(sum(diag(single(([gradW; gradH']'*[gradW; gradH'])))));
%fprintf('Init gradient norm %f\n', initgrad); 
tolW = max(0.001,tol)*initgrad; tolH = tolW;

for iter=1:maxiter,
  % stopping condition
  projnorm = norm(single([gradW(gradW<0 | W>0); gradH(gradH<0 | H>0)]));
  projnorm = (svd([gradW(gradW<0 | W>0); gradH(gradH<0 | H>0)]));
