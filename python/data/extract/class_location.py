#Program name : class_location.py
#Author       : LAM Wai Man
#Student ID   : 07010702
#Description  : To extract a list of reviews and their correspinding label for training of classifier.
#               The labels would be : good location, bad location
#Input        : name.txt
#Output       : location.dat dump in "../classification/location" location
#Usage        : $python class_location.py
...
while (neg < 500 or pos < 500 ):
  name = f1.readline()
  if(name==''):
    break
  name = name.strip()
  print name
  where = " where location!=\'NULL\'"
  command = 'select review, location from ' + name + where + ';' 
  con.execute(command) 
  for i in con:
    str = i[0]
