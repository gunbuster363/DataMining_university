#Program name : class_rooms.py
#Author       : LAM Wai Man
#Student ID   : 07010702
#Description  : To extract a list of reviews and their correspinding label for training of classifier.
#               The labels would be : good rooms, bad rooms
#Input        : name.txt
#Output       : rooms.dat dump in "../classification/rooms" location
#Usage        : $python class_rooms.py
...
while (neg < 1100 or pos < 1100 ):
  name = f1.readline()
  if(name==''):
    break
  name = name.strip()
  print name
  where = " where rooms!=\'NULL\'"
  command = 'select review, rooms from ' + name + where + ';' 
  con.execute(command) 
  for i in con:
    str = i[0]
    ...
    if(i[1]>3 and pos<1100):
     tuple = (tokens,"good room")
     documents.append(tuple)
     pos+=1

    if(i[1]<3 and neg<1100):
     tuple = (tokens,"bad room")
     documents.append(tuple)
     neg+=1
...
for w in dict:
 print w, dict[w]
f1.close()
fout = open("../classification/rooms/rooms.dat", "wb")
